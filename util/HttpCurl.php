...
class HttpCurl
{
    ...
    public function getJson($url,$json, $referer=''){
        return $this->_requestJson('GET', $url, $json,  $referer);
    }
    ...
    public function postJson($url,$json, $referer=''){
        return $this->_request('POST', $url, $json, $referer);
    }
    ...
    private function _request($method, $url, $params=array(), $uploadFile=array(), $referer=''){
        ...
        curl_setopt($this->ch, CURLOPT_CUSTOMREQUEST, $method);//一定要指定命令，ＲＥＳＴ中才能通过
    ...
    private function _requestJson($method, $url, $jsondata, $referer=''){
